  ...
  class Node extends BitMapTree {
    ...
    allocate(counter, callback) {
      ...
      if (index === null || index < 0 || index >= blockSize) {
        ...
        this.bitMapTrees[index].allocate(counter, (counter, bitMap) => {
        ...
        child.allocate(counter, (counter, bitMap) => {
          if (bitMap && allSet(bitMap)) {
            setBit(this.bitMap, index);
          }
          callback(counter, this.bitMap);
...
class Counter {
  ...
  allocate(counter) {
    ...
    this._bitMapTree.allocate(index, (index_, bitMap) => {
      index = index_;
    });
    if (index !== null) {
   ...
   */
  deallocate(counter) {
    this._bitMapTree.deallocate(counter - this._begin, () => {});
  }
